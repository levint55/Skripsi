...
Pada bagian ini akan dijelaskan secara singkat untuk meningkatkan kepatuhan aplikasi \textit{SharIF Judge} menjadi \textit{level A} dengan cara memenuhi tingkat kepatuhan \textit{level A} pada bagian \ref{sec:kepatuhan_sharif_judge_terhadap_wcag_2.1}.
...
\begin{itemize}
	\item Logo \textit{SharIF Judge} pada bagian menu atas.
	\item Gambar \textit{PDF} pada halaman \textit{Assignment}.
\end{itemize}

\subsubsection{Kriteria Sukses 1.3.1 Info dan Relationships}
\label{subsubsec:peningkatan_A_1.3.1}

Tanya : Apakah isi tabel harus bisa difokus agar screen reader dapat membaca isi tabel.
...
\begin{itemize}
	...
	\item \textit{Dropdown} pada logo \textit{Profile} pada bagian menu atas.
...
\begin{itemize}
	\item Seluruh gambar pada \textit{sidebar}.
	\item Gambar \textit{Profile} pada bagian menu atas.
\end{itemize}
...
Kriteria ini dapat dipatuhi dengan cara memastikan setiap elemen memiliki \textit{tag} buka dan tutup yang lengkap, elemen disusun berlapis sesuai spesifikasinya, elemen tidak memiliki atribut duplikat, dan setiap elemen memiliki ID yang unik. Elemen yang dimaksud adalah:
